...
Valor: R$ ${readyPrice?.toFixed(2).replace(".", ",")}`,
  ...
  return (
    <Link href={`/produto/${formatPath(product.name)}`} className="group block">
      <article className="border-border/80 bg-card relative overflow-hidden rounded-[1.75rem] border transition-all duration-500 hover:-translate-y-1 hover:shadow-xl">
        <ProductImage
          product={product}
          ...
          <div className="border-border/70 mt-6 flex items-end justify-between gap-4 border-t pt-5">
            ...
            {isReadyProduct ? (
              <button
                ...
                className="bg-primary flex cursor-pointer items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold text-white transition-all duration-300 hover:scale-[1.02] hover:shadow-md"
              >
