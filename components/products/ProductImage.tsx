...
export default function ProductImage({
  ...
  return (
    <div
      className="relative aspect-9/12 w-full overflow-hidden bg-muted"
    ...
    >
      ...
      {images.length > 1 && (
        <div className="pointer-events-none absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-background/75 px-2.5 py-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-300 group-hover:opacity-100">
          {images.map(
            (_, index) => (
              <span
                key={index}
