...
function normalizeProduct(product: RawProduct): Product {
  ...
  const images = Object.fromEntries(
    Object.entries(product.images).filter(([, images]) =>
...
const products: Product[] = productsData.products.map(normalizeProduct);
...
export function getBestSellingByCategory(): Record<string, Product> {
...
export function getReadyProducts(): ReadyProduct[] {
  return productsData.prontaEntrega
    ...
    .filter((product): product is ReadyProduct => product !== null);
}
